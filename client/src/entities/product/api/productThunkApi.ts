...
export const createProduct = createAsyncThunk<
  ...
  { rejectValue: IApiResponseReject }
>(
  PRODUCTS_THUNKS_TYPES.CREATE_PRODUCT,
  async (newProduct, { rejectWithValue }) => {
    try {
      const { data } = await axiosInstance.post<IApiResponseSuccess<IProduct>>(
        `${PRODUCTS_API_ROUTES.PRODUCTS}`,
        newProduct
      );
      return data;
...
export const deleteProductById = createAsyncThunk<
  ...
  { rejectValue: IApiResponseReject }
>(
  PRODUCTS_THUNKS_TYPES.DELETE_PRODUCT_BY_ID,
...
>(
  ...
  async ({ id, productData }, { rejectWithValue }) => {
    try {
      const { data } = await axiosInstance.put<IApiResponseSuccess<IProduct>>(
        `${PRODUCTS_API_ROUTES.PRODUCTS}/${id}`,
        productData
      );
      return data;
